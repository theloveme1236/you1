...
def follow_tiktok():
    ...
    for s in range(999999999999999):
        try:
            ...
            current_url = driver.current_url
            print(current_url)
            #subprocess.run('adb connect 192.168.1.4:5555', shell=True, capture_output=True, text=True)
            #time.sleep(2)
            subprocess.run('adb shell am start -a android.intent.action.VIEW -d "{}"'.format(current_url), shell=True, capture_output=True, text=True)
            time.sleep(6)
            for a in range(9):
                subprocess.run('adb shell input keyevent KEYCODE_TAB', shell=True, capture_output=True, text=True)

            subprocess.run('adb shell input keyevent 66', shell=True, capture_output=True, text=True)
            time.sleep(2)
            #subprocess.run('adb shell am force-stop com.zhiliaoapp.musically'.format(current_url), shell=True, capture_output=True, text=True)
            driver.close()
            driver.switch_to.window(driver.window_handles[0])
            time.sleep(2)
            driver.find_element(By.CSS_SELECTOR, '[alt="Click On The Button To Confirm Interaction!"]').click()
